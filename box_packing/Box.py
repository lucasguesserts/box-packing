...
class Box:
    def __init__(self, id, length, width, height):
        ...
        self.rotation =  [Rotation(*p) for p in list(set(itertools.permutations(self.dimension, 3)))]
...
class Pallet:
    ...
    def fill(self, box_list):
        for x, y, z in [(x,y,z) for x in range(self.dimension[0]) for y in range(self.dimension[1]) for z in range(self.dimension[2])]:
            if(self.space[x,y,z] == 0): # space available
                for idx in box_list.available_boxes(): # get a box type
                    for rotation in box_list.box(idx).rotation: # get a rotation of the box
                        if self.add(box_list.box(idx).id, rotation, [x,y,z]): # try to add the rotation at that space
...
class BoxList:
    def __init__(self, box_list_initializer):
        ...
        for box_id, box_dimension, box_count in box_list_initializer:
            self.listing.append([
                Box(box_id, *box_dimension),
                box_count
            ])
        self.listing = sorted(self.listing, key=lambda entry: entry[0].volume, reverse=True) # big boxes first
